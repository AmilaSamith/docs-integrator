...
|-------|------|---------|-------------|
| `auth` | `http:BearerTokenConfig`\|`OAuth2RefreshTokenGrantConfig` | Required | Authentication configuration. Supply either a Personal Access Token as a bearer token (`{token: ""}`) or GitHub App credentials for the OAuth2 refresh token grant. See [Authentication](#authentication). |
