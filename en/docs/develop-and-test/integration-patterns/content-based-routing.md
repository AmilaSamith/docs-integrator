...
sidebar_position: 3
slug: /develop-and-test/integration-patterns/content-based-routing
---
...
Use pattern-based content routing with [match expressions](../../editor/canvases/flow-canvas/node-palette.md#match) when each recipient maps to a known field value, such as a message type, region, product category, or event name. Keep an explicit default branch so unsupported content is handled in a dedicated fallback path for invalid recipients.
...
Use predicate-based content routing with [if/else statements](../../editor/canvases/flow-canvas/node-palette.md#if) when the route depends on content rules instead of one stable routing key.
