...
1. Add the connector client connection for the application channel. See [adding a connection](../integration-artifacts/supportive-artifacts/connections.md#adding-a-connection); for this example, select the Jira connector as shown in [adding the Jira connector](../../connectors/catalog/productivity-collaboration/jira/example.md#adding-the-jira-connector).
...
An HTTP service adapts an inbound HTTP channel into an integration flow. Define the service resource as the adapter entry point, receive a typed request payload, and return the typed response expected by the caller. See [creating an HTTP service](../integration-artifacts/integration-as-api/http.md#creating-an-http-service) for the service setup flow.

<PatternImplementationTabs>
<TabItem value="ui" label="Visual Designer" default>

1. Create an HTTP service for the inbound channel. See [creating an HTTP service](../integration-artifacts/integration-as-api/http.md#creating-an-http-service).
2. Add the resource that represents the inbound adapter operation. Use [resource inputs](../integration-artifacts/integration-as-api/http.md#defining-inputs) to define the request payload or parameters.
3. Define the response payload type for the resource with [response schemas](../integration-artifacts/integration-as-api/http.md#defining-response-schemas).
...
Use a broker listener when the channel is a messaging broker rather than an application API or HTTP endpoint. The service receives records from the broker, converts each record into the flow payload, and acknowledges or publishes through the connector according to the channel contract. Use the relevant broker guide, such as [Kafka consumers](../integration-artifacts/event-driven-integration/kafka.md#creating-a-kafka-listener), [RabbitMQ services](../integration-artifacts/event-driven-integration/rabbitmq.md#creating-a-rabbitmq-service), or the [JMS listener](../../connectors/catalog/messaging/java.jms/triggers.md#listener).

<PatternImplementationTabs>
<TabItem value="ui" label="Visual Designer" default>

1. Create the broker listener for the channel. For Kafka, see [creating a Kafka consumer](../integration-artifacts/event-driven-integration/kafka.md#creating-a-kafka-listener); for RabbitMQ, see [creating a RabbitMQ service](../integration-artifacts/event-driven-integration/rabbitmq.md#creating-a-rabbitmq-service); for JMS, see the [JMS listener](../../connectors/catalog/messaging/java.jms/triggers.md#listener).
2. Configure the broker endpoint, topic or queue, and credentials with configurables. For Kafka, use [service configuration](../integration-artifacts/event-driven-integration/kafka.md#service-configuration); for RabbitMQ, use [listener configuration](../integration-artifacts/event-driven-integration/rabbitmq.md#listener-configuration).
3. Add the message-handling function for the broker event. For Kafka, use [service configuration](../integration-artifacts/event-driven-integration/kafka.md#service-configuration); for RabbitMQ, use [event handlers](../integration-artifacts/event-driven-integration/rabbitmq.md#event-handlers).
