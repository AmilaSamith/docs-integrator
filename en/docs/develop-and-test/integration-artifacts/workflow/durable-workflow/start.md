...
slug: /develop-and-test/integration-artifacts/workflow/durable-workflow/start
---
...
3. Fill in the form:
   ...
   |---|---|---|
   | **Input** | Yes | The data the workflow starts with. It must match the workflow's [input type](create.md). Pick the request payload from **Inputs** in the value helper. |
...
**Run Workflow** starts the run and returns its ID straight away. It does not wait for the workflow to finish, and that is the point: a durable workflow may sit on a [human task](await-human-task.md) or a [data event](data-events.md) for days, and no caller should be held open for that.
...
| Route | Use it for                                                                             |
|---|----------------------------------------------------------------------------------------|
| [Integration Control Plane](../../../../integrator/icp/icp-console-overview.md) | Starting a run by hand from a generated form, for testing, onboarding, and operations. |

## Next steps

- [Await data events](data-events.md) — deliver data into a run using the ID you kept.
- [Workflow executions](../../../../integrator/icp/icp-console-overview.md) — follow a started run through its timeline and execution graph.
- [Build an order processing workflow](../../../../get-started/quickstarts/build-order-processing.md) — a service resource that starts a workflow, end to end.
