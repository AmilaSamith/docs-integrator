...
keywords: [wso2 integrator, automation, sap business one, sap b1, inventory, purchasing, email, smtp, notification, low stock, reorder, use case, low-code]
slug: /guides/business-use-cases/sap-b1-low-stock-purchase-automation
card_icon: automation
card_summary: A scheduled, no-code automation that watches inventory and raises purchase requests
card_keywords: [erp, procurement, inventory, scheduled task, notification]
---
...
An [automation](../../develop-and-test/integration-artifacts/automation.md#creating-an-automation) runs on a schedule with no inbound request, which makes it the right artifact for a recurring stock check.
...
1. Add a [connection](../../develop-and-test/integration-artifacts/supportive-artifacts/connections.md#adding-a-connection): **Add Connection → Search Connectors**, then search for `sap.businessone.inventory` and select **ballerinax/sap.businessone.inventory**.
...
1. Add a [**Declare Variable**](../../editor/canvases/flow-canvas/node-palette.md#declare-variable) node after **List Items** that assigns `lowStockResult.value ?: []` to a variable named `lowStockItems` (type `Item[]`).
...
2. Add an [**If**](../../editor/canvases/flow-canvas/node-palette.md#if) node after it with the condition `lowStockItems.length() == 0`.
3. Inside the branch, add a **Log Info** node with the message `"No low-stock items to reorder."`
4. After the log, add a [**Return**](../../editor/canvases/flow-canvas/node-palette.md#return) node with no value.
...
1. Add a [**Foreach**](../../editor/canvases/flow-canvas/node-palette.md#foreach) node after the **If**, looping over `lowStockItems` with the item variable `lowStockItem` (type `Item`).
...
After the **Foreach** node, add a final [**Log Info**](../../editor/canvases/flow-canvas/node-palette.md#log-info) node with the message `"Done - reordered low-stock items"`.
