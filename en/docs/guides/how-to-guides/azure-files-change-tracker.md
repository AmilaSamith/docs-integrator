...
slug: /guides/how-to-guides/azure-files-change-tracker
card_icon: file
card_summary: A scheduled tracker that reports every created, modified, and deleted file on an Azure Files share
card_keywords: [azure, file share, change tracking, etag, snapshot, scheduled task]
---
...
An [automation](../../develop-and-test/integration-artifacts/automation.md#creating-an-automation) runs on a schedule with no inbound request, which is the right artifact for a tracker: each run checks the share once and exits, so nothing stays resident between checks.
...
1. Add a [connection](../../develop-and-test/integration-artifacts/supportive-artifacts/connections.md#adding-a-connection): select **Add Connection**, open **Search Connectors**, then search for `azure.storage.files` and select **ballerinax/azure.storage.files**.
2. Configure the connection:
    ...
    :::tip Best practice
    Don't hardcode these values into the connection. Click each field and select **Configurables** in the [Expression Panel](../../editor/panels/expression-panel.md)'s helper pane, then click **New Configurable**, so the values are supplied at runtime instead of stored in the flow. The tracker uses four: `shareName`, `accountName`, `accountKey`, and the defaulted `snapshotPath`, which only needs overriding to relocate the snapshot.
    :::
