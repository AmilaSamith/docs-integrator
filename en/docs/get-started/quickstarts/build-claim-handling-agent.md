...
`LOCAL` is one of several deployment modes. A production integration usually points at a self-hosted or managed server instead, which is a matter of configuration rather than a change to the workflow.
...
- [Await human task](../../develop-and-test/integration-artifacts/workflow/durable-workflow/await-human-task.md) — ask people structured questions, not just approvals.
- [Error handling and review activities](../../develop-and-test/integration-artifacts/workflow/durable-workflow/review-activity-and-error-handling.md) — let a human fix a failed step's input and retry it.
- [Create a Durable Agent](../../develop-and-test/integration-artifacts/workflow/durable-agentic-workflow/create-durable-agent.md) — events, multi-turn conversations, and agent-to-agent collaboration.
