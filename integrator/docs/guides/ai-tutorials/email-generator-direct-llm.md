...
This tutorial walks through building an **HTTP service that generates professional emails using an LLM**. It's a complete, end-to-end scenario that exercises the [Direct LLM Calls](../../develop/ai/direct-llm/overview.md) feature surface.
...
2. **Add a [model provider](../../develop/ai/components/model-providers.md)** as the connection to the LLM.
...
> **Tip:** The Default WSO2 Model Provider does not require an API key. For a different provider see [AI Connections and Stores → Model Providers](../../develop/ai/components/model-providers.md).
...
- **[Direct LLM Calls reference](../../develop/ai/direct-llm/overview.md)** -- the single-page feature reference covering the `generate` node, prompt editor, and typed responses.
- **[Model Providers](../../develop/ai/components/model-providers.md)** -- switch the LLM provider for production (init params, supported models, advanced HTTP configs for OpenAI, Azure, Anthropic, Vertex, Mistral, DeepSeek, Ollama, OpenRouter).
